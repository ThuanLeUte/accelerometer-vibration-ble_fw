...
base_status_t hmc5883l_init(hmc5883l_t *me)
{
  ...
  CHECK_STATUS(m_hmc5883l_read_reg(me, HMC5883L_REG_IDENT_A, identifier, 3));

  CHECK(HMC5883L_VALUE_IDENT_A == identifier[0], BS_ERROR);
  CHECK(HMC5883L_VALUE_IDENT_B == identifier[1], BS_ERROR);
  CHECK(HMC5883L_VALUE_IDENT_C == identifier[2], BS_ERROR);
...
base_status_t hmc5883l_set_measurement_mode(hmc5883l_t *me, hmc5883l_mode_t mode)
{
  ...
  CHECK_STATUS(m_hmc5883l_read_reg(me, HMC5883L_REG_MODE, &value, 1));

  value &= 0xFC;
  value |= mode;

  CHECK_STATUS(m_hmc5883l_write_reg(me, HMC5883L_REG_MODE, &value, 1));
...
base_status_t hmc5883l_set_range(hmc5883l_t *me, hmc5883l_range_t range)
{
  ...
  CHECK_STATUS(m_hmc5883l_write_reg(me, HMC5883L_REG_CONFIG_B, &value, 1));
